...
const InserimentoPresenzeApp = (() => {
  ...
  async function handleSaveRows() {
    ...
    try {
      ...
      state.rows = state.rows.map((row) => ({ ...row, dirty: false }));
      saveState();
      renderRowsView();
  ...
  async function loadOperatorsFromDatabase() {
    ...
    try {
      ...
      state.operators = rawOperators.map(mapOperatorRow).filter((row) => row.id !== null || row.nome || row.cognome);
  ...
  function buildAttendanceRow(operator, index) {
    ...
    const finalMin = calculateFinalMinutes(workMin, Number(state.setup.snackMin) || 0, Number(state.setup.stopsMin) || 0, 0, 0, 0);
    return {
  ...
  function validateAllRows() {
    ...
    for (let i = 0; i < state.rows.length; i += 1) {
      ...
      if ((Number(row.work_min) || 0) < 0) return { ok: false, message: `Ore lavorate negative alla riga ${i + 1}.` };
      ...
      if ((Number(row.final_min) || 0) < 0) return { ok: false, message: `Il tempo finale non può essere negativo alla riga ${i + 1}.` };
    }
    return { ok: true };
  }
  ...
  function restoreState() {
    try {
      ...
      if (parsed.setup && typeof parsed.setup === 'object') state.setup = { ...state.setup, ...parsed.setup };
