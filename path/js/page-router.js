/* ==========================================================
   VSL Operations Workforce - Multipage Router V2
   ========================================================== */
(function () {
  'use strict';
  ...
  var NAV_IDS = {
    ...
    plannedOpenAddBtn: 'assenze',
    plannedOpenCalendarBtn: 'assenze',
  ...
  var TOP_NAV_IDS = [
    'openHomeBtn','openAttendanceBtn','openPlannedAbsencesBtn','openOperatorsBtn',
    'openAttendanceAdminBtn','openProgrammingBtn','openTempiMetodiBtn'
  ];
  ...
  function shouldIntercept(id) {
    if (!id || !NAV_IDS[id]) return false;
    // On internal pages the action cards/buttons inside the page must continue to work.
    // Only the home/dashboard cards and top nav are converted to URL navigation.
    if (currentPage() !== 'dashboard' && (id === 'plannedOpenAddBtn' || id === 'plannedOpenCalendarBtn')) return false;
    return true;
  }
  ...
  function installNavigationInterceptor() {
    document.addEventListener('click', function (event) {
      ...
      if (!target || !shouldIntercept(target.id)) return;
  ...
  function forceHomeButtonsToNavigate() {
    Object.keys(NAV_IDS).forEach(function (id) {
      var el = document.getElementById(id);
      if (!el) return;
      el.dataset.pageTarget = NAV_IDS[id];
      // Assigning onclick fixes cases where older handlers override normal bubbling.
  ...
  function applyPageChrome() {
    ...
    TOP_NAV_IDS.forEach(function (id) {
      var el = document.getElementById(id);
      if (!el) return;
      // User request: all area buttons only in Home. On subpages hide upper navigation buttons.
      if (page !== 'dashboard') el.classList.add('vsl-page-hidden-nav');
      else el.classList.remove('vsl-page-hidden-nav');
      el.classList.toggle('is-current-page', NAV_IDS[id] === page);
    });
  }
  ...
  function init() {
    ...
    window.setTimeout(forceHomeButtonsToNavigate, 800);
    window.setTimeout(forceHomeButtonsToNavigate, 1800);
  }
