...
(function () {
    ...
    section.innerHTML = `
      ...
      <div id="programmingDatabasePanel" class="hidden">
        <div class="card inline-card">
          ...
          <div id="programmingProgramsList" class="programming-programs-list"></div>
      ...
      <div id="programmingEditorPanel" class="hidden">
        ...
        <div class="card programming-table-card">
          ...
          <div class="programming-export-live actions">
            <button id="exportEditorLayoutExcelBtn" class="btn btn-secondary" type="button">Scarica Excel layout editor</button>
            <button id="printEditorPdfBtn" class="btn btn-secondary" type="button">Stampa / PDF editor</button>
          </div>
        </div>
      </div>
    `;
  ...
  function bind() {
    const actions = {
      ...
      resetProgrammingEditorBtn: resetEditor,
      exportEditorLayoutExcelBtn: exportEditorLayoutExcel,
      printEditorPdfBtn: printEditorPdf,
    ...
    const list = $('programmingProgramsList');
    if (list && list.dataset.boundProgramming !== '1') {
      list.dataset.boundProgramming = '1';
      list.addEventListener('click', (event) => {
        const button = event.target.closest('button[data-edit-program]');
        if (button) loadProgramIntoEditor(button.dataset.editProgram);
      });
    }
  ...
  async function loadFteRealForDates() {
    state.fteReal = {};
    if (!client || !state.dates.length) return;
    const from = state.dates[0];
    const to = state.dates[state.dates.length - 1];
    try {
      const opRes = await client.from('operators').select('*');
      const operators = opRes.error ? [] : (opRes.data || []);
      const totals = { 'CALZOLERIA 1': 0, 'CALZOLERIA 2': 0 };
      operators.forEach((op) => {
        const line = firstValue(op, ['lineaproduzione', 'lineaProduzione', 'linea_produzione', 'linea', 'line'], '');
        if (!PROGRAM_LINES.includes(line)) return;
        const active = firstValue(op, ['is_active', 'isactive'], true);
        if (active === false) return;
        const fte = num(firstValue(op, ['fte_programmabili', 'fteprogrammabili', 'fteProgrammabili'], 1), 1);
        totals[line] += fte;
      });
      const absRes = await client.from('planned_absences').select('absence_date,line_name,fte_absence_programmabili,hours,standard_absence_hours').gte('absence_date', from).lte('absence_date', to).in('line_name', PROGRAM_LINES);
      const abs = absRes.error ? [] : (absRes.data || []);
      PROGRAM_LINES.forEach((line) => {
        state.fteReal[line] = {};
        state.dates.forEach((d) => {
          const fteAbs = abs.filter((a) => a.absence_date === d && a.line_name === line).reduce((sum, a) => {
            const stored = a.fte_absence_programmabili;
            if (stored !== undefined && stored !== null && stored !== '') return sum + num(stored, 0);
            const h = num(a.hours, 0);
            const std = num(a.standard_absence_hours, 8);
            return sum + (std > 0 ? h / std : 0);
          }, 0);
          state.fteReal[line][d] = Math.max(0, totals[line] - fteAbs);
        });
      });
    } catch (error) {
      console.warn('FTE reali programmazione non disponibili:', error);
    }
  }
  function resetEditor() {
    ...
    state.fteReal = {};
    if ($('programmingTitleInput')) $('programmingTitleInput').value = '';
    if ($('programmingFromDate')) $('programmingFromDate').value = todayIso();
    if ($('programmingToDate')) $('programmingToDate').value = plusDaysIso(todayIso(), 2);
    if ($('saveProgrammingEditorBtn')) $('saveProgrammingEditorBtn').textContent = 'Salva programma';
    renderEditor();
    show($('programmingMessage'), 'Editor ripulito. Puoi selezionare un nuovo periodo.', 'success');
  }
  function renderEditor() {
    ...
    const fteRow = (label, line, cls) => {
      const cells = state.dates.map((d) => {
        const value = state.fteReal && state.fteReal[line] && state.fteReal[line][d] !== undefined ? Number(state.fteReal[line][d]).toFixed(2) : '-';
        return `<td class="fte-cell">${esc(value)}</td>`;
      }).join('');
      return `<tr class="${cls}"><td colspan="4"><strong>${esc(label)}</strong></td>${cells}<td></td><td></td></tr>`;
    };
    const totalCells = state.dates.map((d) => `<td class="total-cell">${esc(state.rows.reduce((s, r) => s + num(r.quantities?.[d], 0), 0))}</td>`).join('');
    wrap.innerHTML = `<table class="attendance-table programming-table"><thead><tr><th>Linea</th><th>Codice forma</th><th>Qta forme</th><th>Articolo</th>${dateHeads}<th>Note</th><th>Azioni</th></tr></thead><tbody>${body}</tbody><tfoot>${subtotalRow('Parziale VSL1', 'CALZOLERIA 1', 'partial-row partial-vsl1')}${fteRow('FTE reali programmabili VSL1', 'CALZOLERIA 1', 'fte-row fte-vsl1')}${subtotalRow('Parziale VSL2', 'CALZOLERIA 2', 'partial-row partial-vsl2')}${fteRow('FTE reali programmabili VSL2', 'CALZOLERIA 2', 'fte-row fte-vsl2')}<tr class="grand-total-row"><td colspan="4"><strong>Totale generale</strong></td>${totalCells}<td></td><td></td></tr></tfoot></table>`;
  }
  ...
  async function saveProgram() {
    ...
    try {
      ...
      const wasEditing = Boolean(state.editingProgramId);
      let programId = state.editingProgramId;
      if (programId) {
        const upd = await client.from('production_programs').update({ title, start_date: state.dates[0], end_date: state.dates[state.dates.length - 1], updated_at: new Date().toISOString() }).eq('id', programId).select().single();
        if (upd.error) throw upd.error;
        const del = await client.from('production_program_daily_rows').delete().eq('program_id', programId);
        if (del.error) throw del.error;
      } else {
      ...
      validRows.forEach((r, rowIndex) => {
        state.dates.forEach((d) => {
          dailyRows.push({
            ...
            article_code: r.lookup_code || r.article_code,
      ...
      if ($('saveProgrammingEditorBtn')) $('saveProgrammingEditorBtn').textContent = 'Aggiorna programma';
      show($('programmingMessage'), `${wasEditing ? 'Programma aggiornato' : 'Programma salvato'} correttamente: ${dailyRows.length} righe giornaliere.`, 'success');
  ...
  function renderProgramDatabase() {
    ...
    renderProgramsList(rows);
    body.innerHTML = rows.map((row) => `<tr><td>${esc(formatDateIT(row.production_date))}</td><td>${esc(row.program_title || '-')}</td><td>${esc(LINE_LABEL[row.line_name] || row.line_name || '-')}</td><td>${esc(row.lookup_code || row.article_code || '-')}</td><td>${esc(row.form_code || '-')}</td><td>${esc(row.form_qty ?? '-')}</td><td>${esc(row.quantity ?? 0)}</td><td>${esc(row.note || '-')}</td></tr>`).join('') || '<tr><td colspan="8">Nessun programma trovato con i filtri selezionati.</td></tr>';
  }
  function renderProgramsList(rows) {
    const box = $('programmingProgramsList');
    if (!box) return;
    const map = new Map();
    rows.forEach((row) => {
      const id = row.program_id || 'senza-id-' + row.program_title;
      if (!map.has(id)) map.set(id, { id, title: row.program_title || 'Programma', from: row.production_date, to: row.production_date, qty: 0, lines: new Set() });
      const item = map.get(id);
      if (row.production_date < item.from) item.from = row.production_date;
      if (row.production_date > item.to) item.to = row.production_date;
      item.qty += num(row.quantity, 0);
      item.lines.add(LINE_LABEL[row.line_name] || row.line_name || '-');
    });
    const programs = Array.from(map.values()).sort((a, b) => String(b.from).localeCompare(String(a.from)));
    box.innerHTML = programs.length ? `<div class="program-list-title">Programmi inseriti</div><div class="program-list-grid">${programs.map((p) => `<div class="program-list-card"><strong>${esc(p.title)}</strong><span>${esc(formatDateIT(p.from))} - ${esc(formatDateIT(p.to))}</span><span>${esc(Array.from(p.lines).join(', '))} · ${esc(p.qty)} paia</span><button class="btn btn-secondary btn-small" type="button" data-edit-program="${esc(p.id)}">Modifica</button></div>`).join('')}</div>` : '';
  }
  async function loadProgramIntoEditor(programId) {
    if (!programId || String(programId).startsWith('senza-id')) return;
    hide($('programmingMessage'));
    const res = await client.from('production_program_daily_rows').select('*').eq('program_id', programId).order('production_date', { ascending: true }).order('sort_order', { ascending: true });
    if (res.error) { show($('programmingMessage'), res.error.message || 'Errore apertura programma.', 'error'); return; }
    const data = res.data || [];
    if (!data.length) { show($('programmingMessage'), 'Programma senza righe giornaliere.', 'error'); return; }
    state.editingProgramId = programId;
    state.dates = Array.from(new Set(data.map((r) => r.production_date))).sort();
    state.editorReady = true;
    const grouped = new Map();
    data.forEach((r) => {
      const key = [r.line_name, r.lookup_code || r.article_code || '', r.form_code || '', r.note || '', r.sort_order || 0].join('||');
      if (!grouped.has(key)) grouped.set(key, { line_name: r.line_name, lookup_code: r.lookup_code || r.article_code || '', article_code: r.lookup_code || r.article_code || '', form_code: r.form_code || '', form_qty: r.form_qty ?? '', note: r.note || '', quantities: {} });
      grouped.get(key).quantities[r.production_date] = r.quantity ?? 0;
    });
    state.rows = Array.from(grouped.values()).map(ensureRowQuantities);
    if ($('programmingFromDate')) $('programmingFromDate').value = state.dates[0];
    if ($('programmingToDate')) $('programmingToDate').value = state.dates[state.dates.length - 1];
    if ($('programmingTitleInput')) $('programmingTitleInput').value = data[0].program_title || '';
    if ($('saveProgrammingEditorBtn')) $('saveProgrammingEditorBtn').textContent = 'Aggiorna programma';
    await loadFteRealForDates();
    openProgrammingPage('editor');
    renderEditor();
    show($('programmingMessage'), 'Programma aperto in modifica. Salvando verranno sostituite le righe precedenti.', 'info');
  }
  function exportProgramDatabase() {
    ...
    const out = [['DATA', 'TITOLO', 'LINEA', 'ARTICOLO', 'CODICE FORMA', 'QTA FORME', 'QUANTITA', 'NOTE']];
    rows.forEach((row) => out.push([formatDateIT(row.production_date), row.program_title || '', LINE_LABEL[row.line_name] || row.line_name || '', row.lookup_code || row.article_code || '', row.form_code || '', row.form_qty ?? '', row.quantity ?? 0, row.note || '']));
  ...
  // ===== EXPORT LIVE EDITOR =====
  function editorLayoutRows() {
    const headers = ['LINEA', 'CODICE FORMA', 'QTA FORME', 'ARTICOLO'].concat(state.dates.map((d) => formatDateHeader(d).date + ' ' + formatDateHeader(d).weekday)).concat(['NOTE']);
    const rows = [headers];
    state.rows.forEach((r) => {
      rows.push([
        LINE_LABEL[r.line_name] || r.line_name || '',
        r.form_code || '',
        r.form_qty || '',
        r.lookup_code || r.article_code || '',
        ...state.dates.map((d) => num(r.quantities?.[d], 0)),
        r.note || ''
      ]);
    });
    const partial = (label, line) => [label, '', '', '', ...state.dates.map((d) => state.rows.filter((r) => r.line_name === line).reduce((sum, r) => sum + num(r.quantities?.[d], 0), 0)), ''];
    const fte = (label, line) => [label, '', '', '', ...state.dates.map((d) => state.fteReal && state.fteReal[line] && state.fteReal[line][d] !== undefined ? Number(state.fteReal[line][d]).toFixed(2) : '-'), ''];
    rows.push(partial('PARZIALE VSL1', 'CALZOLERIA 1'));
    rows.push(fte('FTE REALI PROGRAMMABILI VSL1', 'CALZOLERIA 1'));
    rows.push(partial('PARZIALE VSL2', 'CALZOLERIA 2'));
    rows.push(fte('FTE REALI PROGRAMMABILI VSL2', 'CALZOLERIA 2'));
    rows.push(['TOTALE GENERALE', '', '', '', ...state.dates.map((d) => state.rows.reduce((sum, r) => sum + num(r.quantities?.[d], 0), 0)), '']);
    return rows;
  }
  function editorLayoutHtmlDocument(forPrint) {
    const rows = editorLayoutRows();
    const title = $('programmingTitleInput')?.value || `Programma ${state.dates[0] || ''} - ${state.dates[state.dates.length - 1] || ''}`;
    const tr = rows.map((row, idx) => {
      const isHead = idx === 0;
      const cls = idx >= rows.length - 3 ? 'total-row' : '';
      const cells = row.map((cell) => `<${isHead ? 'th' : 'td'}>${esc(cell)}</${isHead ? 'th' : 'td'}>`).join('');
      return `<tr class="${cls}">${cells}</tr>`;
    }).join('');
    return `<!doctype html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>
      body{font-family:Arial,sans-serif;margin:${forPrint ? '18px' : '0'};color:#061d2b}h1{font-size:20px;margin:0 0 10px}.priority{background:#7ec9e3;border:1px solid #111;padding:10px;text-align:center;font-weight:800}table{border-collapse:collapse;width:100%;font-size:12px}th{background:#c9edf8;border:1px solid #111;padding:8px;text-align:center}td{border:1px solid #111;padding:8px;text-align:center}td:nth-child(4){font-weight:800}.total-row td{background:#c7c7c7;font-weight:900}.meta{margin:0 0 12px;color:#4b647c}@media print{@page{size:landscape;margin:10mm}button{display:none}}
    </style></head><body><h1>${esc(title)}</h1><p class="meta">Periodo: ${esc(formatDateIT(state.dates[0]))} - ${esc(formatDateIT(state.dates[state.dates.length - 1]))}</p><div class="priority">ATT.NE SEGUIRE PRIORITA' TEMI COME DA DIRETTIVE V.SPA</div><table>${tr}</table></body></html>`;
  }
  function exportEditorLayoutExcel() {
    if (!state.editorReady || !state.dates.length) { show($('programmingMessage'), 'Prima apri l editor periodo.', 'error'); return; }
    const html = editorLayoutHtmlDocument(false);
    const blob = new Blob(['\ufeff' + html], { type: 'application/vnd.ms-excel;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url;
    a.download = 'programma_manovia_layout.xls';
  ...
  function printEditorPdf() {
    ...
    if (!win) { show($('programmingMessage'), 'Popup bloccato: consenti popup per stampare il PDF.', 'error'); return; }
    win.document.open();
    win.document.write(editorLayoutHtmlDocument(true));
    win.document.close();
